...
const mapProgramStringToEnum = (str) => ({ Men: 'Mens', Women: 'Womens' }[str] || null);
...
const mapApiStatusToDbEnum = (status) => {
    if (!status)
        return null;
    ...
    if (lowerCaseStatus === 'open')
        return 'Open';
    if (lowerCaseStatus === 'closed')
        return 'Closed';
    if (lowerCaseStatus === 'complete')
        return 'Complete';
    if (lowerCaseStatus === 'future')
        return 'Future';
    console.warn(`Unknown meet status from API: ${status}`);
    return null;
};
const mapEventIdToName = (eventId, program) => {
    if (!eventId || !program)
        return null;
    const menEvents = {
        '1': 'Floor Exercise',
...
async function syncSanctionInternal(sanctionId, client) {
    console.log(`Fetching full details for sanction ${sanctionId}...`);
    const response = await fetch(`https://api.myusagym.com/v2/sanctions/${sanctionId}`);
    if (!response.ok) {
        console.error(`Failed to fetch sanction ${sanctionId}. Status: ${response.status}`);
        return;
    }
    const data = (await response.json());
    const s = data.sanction;
    const dbMeetStatus = mapApiStatusToDbEnum(s.meetStatus);
    await client.query(`INSERT INTO sanctions (sanction_id, name, start_date, end_date, city, state, site_name, website, program_id, meet_status, has_results, address1, zip, logo_url)
    ...
    for (const club of Object.values(data.clubs)) {
        await client.query(`INSERT INTO clubs (club_id, name, short_name, city, state, zip, website, email, phone)
    ...
    const processedGymnastIds = new Set();
    for (const person of Object.values(data.people)) {
        if (person && person.personId && person.clubId) {
            if (!knownClubIds.has(person.clubId)) {
                await client.query(`INSERT INTO clubs (club_id, name) VALUES ($1, $2) ON CONFLICT (club_id) DO NOTHING;`, [person.clubId, 'Unknown Club (placeholder)']);
            }
            await client.query(`INSERT INTO gymnasts (gymnast_id, club_id, first_name, last_name, gender)
            ...
            console.warn(`Skipping gymnast with missing data: ${person.firstName} ${person.lastName} (ID: ${person.personId})`);
        }
    }
    for (const session of data.sessions) {
        const parsedSessionId = safeParseInt(session.sessionId);
        if (parsedSessionId !== null) {
            await client.query(`INSERT INTO sessions (session_id, sanction_id, name, session_date, program)
            ...
            console.warn(`Skipping session with invalid sessionId: "${session.sessionId}" for sanction ${sanctionId}`);
        }
    }
    for (const rs of data.sessionResultSets) {
        const parsedSessionId = safeParseInt(rs.sessionId);
        if (parsedSessionId !== null) {
            await client.query(`INSERT INTO result_sets (result_set_id, session_id, sanction_id, level, division, official)
            ...
            console.warn(`Skipping result_set with invalid sessionId: "${rs.sessionId}" for sanction ${sanctionId}`);
        }
    }
    for (const sp of Object.values(data.sanctionPeople)) {
        ...
        if (parsedSessionId !== null && processedGymnastIds.has(sp.personId)) {
            await client.query(`INSERT INTO sanction_gymnasts (sanction_id, gymnast_id, session_id, level, division, squad, club_id_for_meet)
            ...
            if (!processedGymnastIds.has(sp.personId)) {
                console.warn(`Skipping sanction_gymnasts link for personId ${sp.personId} because they were not found in the sanction's main 'people' list.`);
            }
            else {
                console.warn(`Skipping sanction_person with invalid sessionId: "${sp.sessionId}" for sanction ${sanctionId}`);
            }
        }
    }
    console.log(`✅ Successfully synced sanction ${sanctionId} and all participants.`);
}
// --- RESOLVERS ---
const resolvers = {
    Query: {
        ...
        sanction: async (_, { sanctionId }) => {
            const res = await pool.query('SELECT sanction_id AS "sanctionId", name, start_date AS "startDate", end_date AS "endDate", city, state, site_name AS "siteName", meet_status, program_id FROM sanctions WHERE sanction_id = $1', [sanctionId]);
            return res.rows[0];
        },
    ...
    Mutation: {
        syncSanctionAndParticipants: async (_, { sanctionId }) => {
            ...
            try {
                ...
                FROM sanctions WHERE sanction_id = $1`, [sanctionId]);
        ...
        syncScores: async (_, { resultSetId }) => {
            ...
            const response = await fetch(`https://api.myusagym.com/v2/resultsSets/${resultSetId}`);
            const data = (await response.json());
            if (!data.scores)
                return [];
            const savedScores = [];
        ...
        syncSterlingGymData: async () => {
            console.log('Starting Sterling Gym data sync for 2022-2023 season onwards...');
            const response = await fetch(`https://api.myusagym.com/v1/meets/past`);
            const allPastMeets = (await response.json());
            ...
            console.log(`Found ${meetsInDateRange.length} meets since ${seasonStartDate.toISOString().split('T')[0]}. Checking for Sterling Gym participation...`);
            const syncedSanctions = [];
            const client = await pool.connect();
            try {
                await client.query('BEGIN');
                for (const meet of meetsInDateRange) {
                    console.log(`Checking sanction ${meet.sanctionId} (${meet.name}) for Sterling Gym participation...`);
                    const sanctionDetailsResponse = await fetch(`https://api.myusagym.com/v2/sanctions/${meet.sanctionId}`);
                    if (!sanctionDetailsResponse.ok) {
                        console.warn(`-- Could not fetch details for sanction ${meet.sanctionId}. Status: ${sanctionDetailsResponse.status}`);
                        continue; // Skip to the next meet
                    }
                    const sanctionData = (await sanctionDetailsResponse.json());
                    const clubIds = Object.keys(sanctionData.clubs || {});
                    if (clubIds.includes(String(sterlingGymClubId))) {
                        console.log(`-- Sterling Gym participated in ${meet.name}. Syncing...`);
                        await syncSanctionInternal(meet.sanctionId, client);
                        syncedSanctions.push({ sanctionId: meet.sanctionId });
                    }
                }
                await client.query('COMMIT');
                console.log(`Finished sync. Synced ${syncedSanctions.length} total meets where Sterling Gym participated.`);
                ...
                const finalRes = await client.query(`
                SELECT sanction_id AS "sanctionId", name, start_date AS "startDate", end_date AS "endDate", city, state, site_name AS "siteName", meet_status, program_id
                FROM sanctions WHERE sanction_id = ANY($1::int[])`, [syncedSanctionIds]);
    ...
    Gymnast: {
        club: async (gymnast) => {
            if (!gymnast.club_id)
                return null;
            const res = await pool.query('SELECT club_id AS "clubId", name, city, state FROM clubs WHERE club_id = $1', [gymnast.club_id]);
            return res.rows[0];
        },
        historicalClub: async (gymnast) => {
            if (!gymnast.club_id_for_meet)
                return null;
            const res = await pool.query('SELECT club_id AS "clubId", name, city, state FROM clubs WHERE club_id = $1', [gymnast.club_id_for_meet]);
            return res.rows[0];
        },
    },
    Session: {
        resultSets: async (session) => {
            const res = await pool.query('SELECT result_set_id AS "resultSetId", level, division FROM result_sets WHERE session_id = $1 AND sanction_id = $2', [session.sessionId, session.sanctionId]);
            return res.rows;
        },
    },
    ResultSet: {
        scores: async (resultSet) => {
            const res = await pool.query('SELECT score_id AS "scoreId", event_id AS "eventId", final_score AS "finalScore", rank, tie, gymnast_id FROM scores WHERE result_set_id = $1', [resultSet.resultSetId]);
            return res.rows;
        },
    },
    Score: {
        ...
        gymnast: async (score) => {
            const res = await pool.query('SELECT gymnast_id AS "gymnastId", first_name AS "firstName", last_name AS "lastName", gender, club_id FROM gymnasts WHERE gymnast_id = $1', [score.gymnast_id]);
            return res.rows[0];
        },
        sanction: async (score) => {
            if (!score.sanction_id)
                return null;
            const res = await pool.query('SELECT sanction_id AS "sanctionId", name, start_date AS "startDate" FROM sanctions WHERE sanction_id = $1', [score.sanction_id]);
            return res.rows[0];
        },
    },
};
